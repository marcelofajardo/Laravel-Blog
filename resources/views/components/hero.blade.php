...
<div class="relative">
  <img src="https://picsum.photos/id/123/200" alt="cover" class="object-cover h-48 w-full rounded" />
  <img src="{{$hero->avatar}}" alt="user" style="left: 50%"
...
<div class="flex justify-between items-start my-4">
  ...
  <div class="p-2" style="max-width: 150px">
    ...
    <h2 class="font-bold my-1">{{$hero->user->name}}</h2>
    <p class="text-xs text-gray-700">Joined {{$hero->created_at->diffForHumans()}}</p>
  </div>

  {{-- HERO PROFILE CONTROLS --}}
  @if ($hero->isOwned())
...
<p class="mb-6 text-sm text-gray-600">
    {{$hero->bio ?? $hero->isOwned() ? "Update your bio.." : "No bio available."}}
</p>
...
<div class="flex justify-between items-center mb-6">
  ...
  <div class="flex">
    <a href="{{$hero->path('edit')}}" class="rounded border border-gray-300 py-2 px-4 text-black text-xs mr-2">
      Edit Profile
    </a>
  </div>
</div>
