    ...
    <div class="mb-6 rounded shadow p-2">
        <div class="flex justify-between items-center">
            ...
            <p class="text-xs text-gray-700 mb-2">
                <a href="{{ route('heroes.show', $post->author_id) }}"
            ...
            <div class="text-xs">
                ...
                <a href="{{ $post->path('edit')}}"
                    class="text-blue-400 hover:text-blue-500 hover:underline mr-1"
                    >Edit
                </a>

                <button class="text-red-400 hover:text-red-500 hover:underline"
                    onclick="event.preventDefault();document.querySelector('#deletePost-{{$post->id}}').submit()"
                    >delete
                </button>
        ...
        @if ($post->image)
        <div>
            <img src="{{$post->image_url}}" alt="post uploaded image"
        ...
        <div class="flex px-2 text-xs text-gray-700 mb-6">
            ...
            {{-- LIKES --}}
            <button class="flex items-center px-3 border border-gray-400 rounded-full
                hover:text-blue-400 hover:border-blue-400
                {{! $post->isOwned() ?: 'cursor-not-allowed' }}
                {{! $post->isLiked() ?: 'text-blue-700'}}"
                @if (! $post->isOwned())
                    onclick="event.preventDefault();document.querySelector('#postLikeForm').submit()"
                @endif
                >
            ...
            @can('like', $post)
                <form id="postLikeForm" action="{{$post->path('like')}}" method="POST" class="hidden">
                    @csrf
                </form>
            @endcan

            {{-- COMMENTS --}}
            <a href="#" class="mx-2 border border-gray-400 rounded-full px-3 cursor-pointer hover:text-blue-400 hover:border-blue-400">
                {{ $post->comments()->count() }}
                <span>Comments</span>
            </a>
